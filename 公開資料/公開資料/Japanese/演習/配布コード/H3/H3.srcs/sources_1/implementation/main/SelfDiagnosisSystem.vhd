...
architecture RTL of SelfDiagnosisSystem is
  ...
  signal reset_shiftreg_icap   : std_logic_vector(kWidthResetSync-1 downto 0);
...
--                dataLocalBusOut   <= status_out.counter_correction(kWidthCorrection-1 downto 8);
  ...
  sync_reset_icap  <= reset_shiftreg_icap(kWidthResetSync-1);
  u_sync_reset_icap : process(rst, clkIcap)
  begin
    if(rst = '1') then
      reset_shiftreg_icap  <= (others => '1');
    elsif(clkIcap'event and clkIcap = '1') then
      reset_shiftreg_icap  <= reset_shiftreg_icap(kWidthResetSync-2 downto 0) & '0';
    end if;
  end process;
