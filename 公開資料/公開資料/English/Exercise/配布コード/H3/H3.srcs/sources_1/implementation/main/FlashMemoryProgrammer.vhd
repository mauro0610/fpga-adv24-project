...
--    SCLK_SPI            : out std_logic;
    ...
    SPID                : in  std_logic_vector(1 downto 0);
...
architecture RTL of FlashMemoryProgrammer is
  ...
  signal reset_shiftreg_spi   : std_logic_vector(kWidthResetSync-1 downto 0);
...
--      sclkSpi     => SCLK_SPI,
  ...
  -- Reset sequence --
  sync_reset_spi  <= reset_shiftreg_spi(kWidthResetSync-1);
  u_sync_reset_spi : process(rst, clkSpi)
  begin
    if(rst = '1') then
      reset_shiftreg_spi  <= (others => '1');
    elsif(clkSpi'event and clkSpi = '1') then
      reset_shiftreg_spi  <= reset_shiftreg_spi(kWidthResetSync-2 downto 0) & '0';
    end if;
  end process;
